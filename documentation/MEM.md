...
| :--- | :--- | :--- | :--- |
| `0x00` | **`ZR`** | Read Only | Hardwired to ground reference. Intercepted by PKF to verify hardware alignment. |
| `0x01` – `0x1F` | **`MIO`** | Read / Write | Reserved for Memory-Mapped Input/Output devices. |
| `0x20` – `0x7F` | **`STD`** | Read / Write | Standard variable storage accessed via **`STR`** and **`LOA`**. Grows upward (incrementing) toward `0xBF`. |
| `0x80` – `0xBF` | **`TMP`** | Read / Write | Buffer (Collision Hazard) zone for large data processing or temporary structural structures. |
| `0xC0` – `0xFF` | **`STK`** | Push / Pop | Dedicated to subroutine contexts. Grows downward (decrementing) toward `0x80`. |
---
...
3. **Hard Overflow** : If **`STD`** goes past `0xBF` or **`STK`** goes past `0x80`, a critical threshold is breached. In this case, the hardware triggers a native system fault to prevent overflow collapse.

> [!CAUTION]
> **Soft Overflow** : If **`STD`** goes past `0x7F` or **`STK`** goes past `0xC0`, it will silently overwrite the **`TMP`** zone.
>The hardware does not generate a fault; memory integrity is purely software-enforced.
>However, a hardware flag is asserted beforehand to indicate whether `RAM[SP-1]` or `RAM[SP+1]` contains an empty value.
